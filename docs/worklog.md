...
### 大設計変更: 極専用 PCB (案 S4) 廃止 + ポゴ確定

**判断**
- **極専用 PCB / 極ストリップ廃止**: 10 カセット + ペンタゴンねじ留め構造の採用により、極周辺 LED の別基板配置をやめ、**全 800 hex を共通 FPC に集約**
  - 各カセット **80 LED**(全 hex)、**5 ストリップ × 160**(80×2)、ESP32 **5 並列出力**
  - 極部は **南極=磁気端子蓋 / 北極=装飾蓋**(LED 無し)。極 PCB / 極チェーン配線 / 極 pillar 内データ通路を削除
  - これに伴い CLAUDE.md §1/2.1/2.2/2.4/2.6/2.7/2.8/3/8.1 と docs 01/02/03 を全面リファクタ(古い 79/790/158/6 ストリップ/polar_pcb 記述を一掃)
- **ポゴピン確定**: RTLECS 2.54 DIP(**1.5A/pin, ストローク 2.0mm, ばね 75gf, 7mm 高**)を inner_deck の **FR4 補強材**で支持(DIP + 補強材)
- **輝度上限運用 / 全白禁止**: 5V 容量制約に合わせ全白(~数 A)を出さず輝度上限を設ける
- **inner_deck = 6 パッド確定**: `2×GND / 2×5V / DIN / DOUT`(回文配列)。電源 2 重化で容量 3A・接点冗長・電圧降下半減。スロット 12.7mm、全 **60 ポゴ**(北30+南30)
  - 検討の結果: シリコン線+コネクタ案は**不採用**(ホットスワップ崩壊・組立煩雑)。電源補強は「広い GND/5V 銅ベタ + ポゴ 2 重化」で対応
  - 6 ピンの問題点 = 圧着力 4.5kgf(Q54 テコ検証と併せ確認)のみ、致命的問題なし
  - 電圧降下の主因は FPC 5V トレース幅。DIN/DOUT 中央注入で最遠 LED までの距離を半減

---

## 2026-06-05 / 06 — FPC 平面化と N/S 合同性の訂正(重要)

**FPC 平面化を polyhedral unfold → MDS に置換(2026-06-02 記述を撤回)**

- 紙試作で「FPC が殻の穴に合わない」ことが判明。原因は2つ:
  1. **カセット分割の不一致**: `generate_fpc_chain.py` が殻(`blender_make_cassettes.py`, `AZ_SHIFT_DEG=54°`)と違う分割(+36°)を使い、80 hex のうち 16 が別面だった → 分割式を殻に一致させて解決。
  2. **polyhedral unfold は非可展面で破綻**: 半ゴアは Gauss 曲率 ≈ 54° を囲み、チェーン蛇行展開は全島間で**最大 14.7mm** ずれた(「歪み 0.26%」はチェーン隣接のみの誤指標)。**古典 MDS**(全島間距離を最小歪み埋込)+ 外側視点整列に置換 → 穴との対応 **最大 0.41mm**。
- ソルバに連結性プルーニング追加(赤道↔赤道一筆書きを高速化)。端点は両端赤道接触を自動選択(cas0: DIN=409/DOUT=418)。
- tab を **15mm 撓みリード ×2 + 矩形ヘッド(補強材ゾーン)**に。KiCad は J1/J2(3 ピン)を pin2 基準で配置。

**N/S は鏡像ではなく proper 回転で合同(2026-06-05 確定)**

- 一時「北↔南は鏡像 → FPC 2 種」と誤結論(frame ベース検証のバグ)。
- 厳密検証(重心保存 O(3) 写像を網羅): cas0(北)→cas5(南) は **赤道軸 180° 回転、det=+1・残差 0.0000mm・全単射**。全カセット proper 回転で合同。
- → **共通 Gerber 1 種 × 10**(元の前提どおり)。南は同一基板を 180° 回して装着、DIN/DOUT 入替は回文 pad + マザーリングクロスが吸収。
- 詳細: [`02-fpc-kicad.md`](02-fpc-kicad.md) Q63 / Q68。
