...
- **北 ↔ 南**: rotoreflection(回反)で結ばれる **鏡像(enantiomorph)**。proper 回転では不一致([Q68](#open-questions--未確定事項))。**「1 Gerber ×10」は不可 → 北 + 鏡像南の 2 種**。電気は回文 pad + マザーリングクロスで吸収するが、両面実装の表裏(LED 面)は別問題
