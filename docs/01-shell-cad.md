...
- **LED 総数: 800** = 全 800 六角形 (12 五角形は全て LED 無し)
  - 各カセット = **80 hexagon = 80 LED** (10 カセット × 80 = 800) ✓ きれいに割れる
  - ペンタゴン 12 個 = LED 無し: 非極 10 (M2.5 ねじ穴) + 極 2 (南極=磁気端子 / 北極=装飾蓋)
  - **極専用 PCB は廃止 (2026-06-02)** — 極周辺 hex も共通 FPC の一筆書きに含む
...
## Pole Assembly / 極部構造 (案 K_new、極専用 PCB 廃止)

### 全体構成 / Overall topology

**案 K_new (確定)**: 極ねじを廃止し、**10 個の非極ペンタゴン位置を M2.5 ねじ留めアンカー** とする分散クランプ方式。
**極専用 PCB は廃止 (2026-06-02)** — 全 hex を共通 FPC に集約。極部は **南極=磁気端子 / 北極=装飾蓋** のみ (LED 無し)。

```
                ┌─────────────────┐
                │  北極装飾蓋      │  ← LED 無し、pillar にスナップ留め
                └──────────┬──────┘
                           │
              ┌────────────┴────────────┐
              │                            │
   ...
   ━━━━━━━━━━━┥    赤道マザーリング (ドーナツ)  ┝━━━━━━━━━━━
              ...
              └──────────────┰─────────────┘
                             ┃
                ┌────────────┸──┐
                │ 南極磁気端子蓋 │   ← Φ4 磁石 + 端子パッド、5 爪スナップ、LED 無し
                └───────────────┘
```
...
- **アーキテクチャ**: **案 K_new (10 非極ペンタゴンねじ × M2.5)**。極専用 PCB は廃止 (全 hex 共通 FPC)
...
- **短 pillar × 2** (北極/南極):
  ...
  - **役割**: 南極=磁気端子蓋の支持 + 配線通路 / 北極=装飾蓋の支持 (構造クランプ荷重は受けない)
  - 南極 pillar: 内部 Φ3 mm 配線通路 (AWG26 × 2 = 磁気端子電源)
  - 北極 pillar: 配線通路不要 (極 LED が無いため)
  - **キャップとの結合はスナップ留め** (中央ねじ無し)
- **極キャップ × 2 (スナップ式、LED 無し)**:
  - 5 cantilever 爪 (ペンタゴン頂点位置) で pillar にパチンと留まる、工具不要、graceful failure (1 爪折れても OK)
...
- 爪の barb (返し) が pillar 先端の縁リップに掛かる
...
| `back_gore` | **薄い (1mm) Goldberg ハーフゴア**の裏当て。FPC を裏から押し付け挟持 | **全面**でテープレス狙い。hex 穴で 0603 逃げ |
| `ring_claw` | マザーリングを掴む**爪** | 位置決め + 抜け止め + 圧着分担 |
| `anchor_post` | 外殻裏の**肉抜きギャップ**に生やす小突起 | FPC 基準穴と兼用。後付けパーツの固定先 |

### Confirmed decisions / 確定事項

- **FPC 固定 = 全面 `back_gore`(機械式・テープレス狙い)** (2026-06-04 更新、試作検証待ち)
  - **薄い (1mm) Goldberg ハーフゴア**を FPC 裏全面に当て、カセットへ押し付けて挟持 → アセテートテープは**予備/任意**に降格(接着剤ゼロは不変)
  - **0603 逃げ**: back_gore の **hex 穴を各 LED と一致**させ 0603 を穴に収める → 押し圧は島間 web/bridge のみ
  - FPC を外向きに押す → LED がラッパ穴に座る(位置決め兼用)。固定点間で浮かないよう保持点を分散(必要なら gore 半径を僅かに大きく予圧)
...
4. `inner_deck` 取付 = **3 点止め: ダボ ×2(カセットの未貫通ダボ穴へ)+ ネジ ×1(Φ2.2 貫通、M2.5/M2 セルフタップ)** → ポゴ DIP 配線。固定点は hex 交点(島の隙間)に配置 (2026-06-04 実装確認)
5. `back_gore`(全面・1mm Goldberg ハーフゴア)取付 = hex ギャップのスナップ/ネジで分散保持 → FPC を全面挟持(テープレス、テープは予備)
...
- **ポゴピンスロット**: ピッチ 2.54mm。当初図は 10.16mm(4pin)だが **6 ピン採用で 12.7mm に拡幅**([CLAUDE.md §3 Q3](../CLAUDE.md#3-open-questions--未確定事項))
- 周囲に取付/位置決め穴
...
- **Q56 (確定)**: `inner_deck` のパッド = **6 極/カセット = `5V-GND-DIN-DOUT-GND-5V`** (2026-06-04 確定)。
  - 回文・データ隣=GND(シールド)。**左3→チェーン始端(DIN/LED01)/ 右3→終端(DOUT/LED80)**に接続
  - **5V/GND を両端 2 枝給電**(左→始端バス / 右→終端バス)→ バス両端給電で **IR ドロップ半減** + 容量2倍 + 接点冗長
  - 一筆書きの DIN(start) と DOUT(end) を**赤道中央で隣接**させる方針 ([fpc Q62](02-fpc-kicad.md)) のため、各カセットが赤道側に**データ2端子**を持つ
  - 北 DOUT → マザーリング → 南 DIN のクロスルーティングは、データ 1 極をマザーリング内配線で受け渡し
  - ピッチ 2.54mm 確定的。6 ピンは **スロット 12.7mm** (= 5×2.54、当初図の 10.16=4pin から拡幅)
  - 全体ポゴ数: 6 極 × 10 カセット = **60 ポゴ** (北30 + 南30、マザーリング表裏に金パッド)
  - ポゴ: RTLECS 2.54 DIP (1.5A/pin, ストローク 2.0mm, ばね 75gf, 高 7mm) を inner_deck の FR4 補強材で支持
- **Q62b (NEW)**: マザーリングのポゴ zone 数 — ユーザー提示図は **4 回対称**だが、5 経度スライス設計では **5 zone** が必要 (各 zone: 表面=北カセット 6 パッド / 裏面=南カセット 6 パッド)。図は概念スケッチか、設計変更か要確認
- **Q57**: `anchor_post` の本数と位置 (肉抜きギャップのどこに何本、FPC 基準穴と兼用)
- **Q58 (方針更新): 全面 `back_gore` でテープレス固定**(試作検証待ち)
  - **板厚**: 1mm Goldberg ハーフゴア(柔軟に追従しつつ押し付け)
  - **0603 逃げ**: hex 穴を各 LED と一致 → 0603 は穴内、押し圧は島間 web/bridge のみ(hex 穴サイズは 0603+半田を逃がす)
  - **保持点数/位置**: 固定点間で FPC が浮かない数を確保(外周 + 内部の hex ギャップに分散スナップ/ネジ)。要試作で均一圧を実測。必要なら gore 半径を僅かに大きくして予圧
  - **クリアランス**: inner_deck・ペンタゴンねじ・ダボ/ネジ位置は gore 側で開口
  - **検証**: 「テープ無しで FPC が浮かないか」を試作実測 → OK なら §2.5 を全面 back_gore で確定
